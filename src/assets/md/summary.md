...
* 二维数组就是数组嵌套,如下代码块1所示: a数组中的每个元素在for循环后被赋值为b数组,循环结束后 a数组变为二维数组

* ```js
  let a = [0,1,2,4,5] // 先声明一维
  let b = ['a','b','c','d'] // 先声明一维
  ...
  // 展开Array(4) 就是 ["a", "b", "c", "d"]
  console.log(a[2][3]) // 第三行第4个 打印d
  ```

* ```js
  // 未知数组长度方法
  let myarr = new Array() //先声明一维 
  for (let i = 0; i < 5; i++) { //一维长度为5
    myarr[i] = new Array() //再声明二维 
    for (let j = 0; j < 4; j++) { //二维长度为4
